...
<div class="flex flex-col gap-4">
	...
	<div class="flex justify-evenly flex-wrap gap-1">
		{#each appManagers as appManager}
			<a
				href={`/settings/authorized-apps/${appManager.app.id}`}
				class="card card-hover container bg-initial overflow-hidden flex flex-col w-[420px] justify-self-center"
			>
				<header class="min-h-[210px]">
					<img
						src={appManager.app.banner}
						alt="User Banner"
							...
							<div class="flex gap-4 items-center">
								...
								<div class="flex flex-col gap-2 items-center">
									<small class="btn btn-sm variant-ghost-primary">
										Creator: {appManager.app.ownerUser.username}
									</small>
									{#if appManager.app.supportServer != ''}
										<small class="mr-11 btn btn-sm variant-ghost-primary">
											Support Server<a
												href={appManager.app.supportServer}
												target="_blank"
	...
	{#if appManagers.length < 1}
		<p class="p text-error-500">You have no authorized apps.</p>
	{/if}
</div>